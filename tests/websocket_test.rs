...
use webscoket_realtime_prac::handlers::{
    create_matching, join_matching, ws_handler, MatchingSessions, WsChannels,
};
...
async fn test_websocket_connection() {
    ...
    assert!(ws_result.is_ok(), "WebSocket connection failed: {:?}", ws_result);
...
async fn test_ready_with_model_message() {
    ...
    let send_result = write.send(Message::Text(ready_msg.to_string().into())).await;
    assert!(send_result.is_ok(), "Failed to send Ready message: {:?}", send_result);
}
...
async fn test_input_move_message() {
    ...
    let send_result = write.send(Message::Text(input_msg.to_string().into())).await;
    assert!(send_result.is_ok(), "Failed to send Move input: {:?}", send_result);
}
...
async fn test_opponent_gets_character_selection_message() {
    ...
    matching_sessions.lock().unwrap().insert(matching_id, session);
    ...
    // 2. Player Bとして接続
    let url_b = format!("ws://127.0.0.1:{}/ws?player_id={}&matching_id={}", port, player_b_id, matching_id);
    let (ws_stream_b, _) = connect_async(&url_b).await.expect("Player B failed to connect");
    let (_, mut read_b) = ws_stream_b.split();

    // 3. Player Aとして接続
    let url_a = format!("ws://127.0.0.1:{}/ws?player_id={}&matching_id={}", port, player_a_id, matching_id);
    let (ws_stream_a, _) = connect_async(&url_a).await.expect("Player A failed to connect");
    ...
    write_a.send(Message::Text(ready_msg.to_string().into())).await.unwrap();

    // 6. Player Bが `OpponentCharacterSelected` メッセージを受信することを確認
    let msg_result = timeout(Duration::from_secs(2), read_b.next()).await;
    assert!(msg_result.is_ok(), "Player B did not receive message in time");

    if let Some(Ok(received_msg)) = msg_result.unwrap() {
        match received_msg {
            Message::Text(text) => {
                let msg: WsMessage = serde_json::from_str(&text).expect("Failed to parse message for B");
                match msg {
